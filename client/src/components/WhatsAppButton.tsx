...
interface WhatsAppButtonProps {
  phoneNumber: string; // 格式：+852XXXXXXXX（不含空格或符號）
  message?: string; // 預設訊息
}
...
export default function WhatsAppButton({ phoneNumber, message }: WhatsAppButtonProps) {
  ...
  return (
    ...
    >
      ...
      {/* Pulsing Ring Animation — rendered BEFORE button so button sits on top */}
      <motion.div
      ...
      <motion.a
        ...
        className="flex items-center justify-center w-16 h-16 rounded-full shadow-lg transition-all duration-300 hover:shadow-2xl hover:scale-110"
        style={{ backgroundColor: "#25D366" }}
